...
public abstract class QuickHullTest {
	...
	protected int oneTimePointCap = 10000;
	...
	private void writePointsToX3Dfile(String filename, String x3dFileHead,
		...
		while(iter.hasNext()) {
			...
			if(iter.hasNext()){
				...
				pointString.append(curr.getX() + " " + curr.getY() + " " + curr.getZ() + ", ");
			}
			else {
				colorsString.append(oneoneone);
				pointString.append(curr.getX() + " " + curr.getY() + " " + curr.getZ());
	...
	protected List<Point> getRandomSphereGaussianPoints(long pointsCount,
		...
		for (int i = 0; i < pointsCount; ++i) {
			...
			double tempZ = 1.0;
			if (is3D) tempZ = rand.nextGaussian();

			
			double len = Math.sqrt(tempX * tempX + tempY * tempY);
			if (is3D)  len = Math.sqrt(tempX * tempX + tempY * tempY + tempZ * tempZ);
	...
	protected List<Point> getRandomSpecialSphereGaussianPoints(long pointsCount,
			boolean is3D) {
		List<Point> pointList = new LinkedList<Point>();
