...
public class QuickHull2D extends QuickHull {
	...
	public List<Point> getBorderPoints(List<Point> pointSetInput) {
		...
		splitInUpperAndLower(leftSidePoint, rightSidePoint, pointSet, borderPoints);

		return borderPoints;
	}

	/**
	 * Teilt eine gegebene Liste an Punkten in zwei weitere
	 * Listen auf, welche zum einen die oberen Punkte repräsentiert 
	 * und zum anderen die unteren Punkte. Die Teilung erfolgt anhand der
	 * Linie die von den beiden Punkten, die am weistesten rechts bzw. links liegen,
	 * aufgespannt wird.
	 * Durch diese beiden Listen werden dann mit dem Quickhull-Algorithmus
	 * alle Randpunkte berechnet. Die Randpunkte werden in einer übergebenen
	 * Liste gespeichert.
	 * 
	 * @param leftSidePoint der Punkt de am weitesten links liegt
	 * @param rightSidePoint der Punkt der am weitesten rechts liegt
	 * @param currentPointSet die Liste der Punkte
	 * @param borderPoints die zu befüllende Liste, welche die Randpunkte darstellt
	 */
	protected void splitInUpperAndLower(Point leftSidePoint,
		...
		calculateBorder(leftSidePoint, rightSidePoint, upperSet,
				borderPoints);

		borderPoints.add(rightSidePoint);
		calculateBorder(rightSidePoint, leftSidePoint, lowerSet,
				borderPoints);
	}

	/**
	 * Berechnet wie weit ein gegebener Punkt von der Linie entfernt liegt,
	 * die durch die anderen beiden übergebenen Punkte aufgespannt wird.
	 * Der Rückgabewert ist ein Maß für die Distanz. Je weiter der Wert von 0
	 * entfernt ist, desto weiter ist der Punkt von der Linie entfernt.
	 * 
	 * @param a erster Punkt um Linie aufzuspannen
	 * @param b zweiter Punkt um Linie aufzuspannen
	 * @param upperCheck Punkt, dessen Abstand berechnet werden soll.
	 * @return
	 */
	 ...
	 * 
	 * @param leftSidePoint linker Punkt der aufzuspannenden Linie
	 * @param rightSidePoint rechter Punkt der aufzuspannenden Linie
	 * @param pointSet momentane Liste der Punkte
	 * @param borderPoints Liste der Randpunkte
	 */
	protected void calculateBorder(Point leftSidePoint,
			Point rightSidePoint, List<Point> pointSet,
			List<Point> borderPoints) {
		...
		Point uppestPoint = getUppestPoint(leftSidePoint, rightSidePoint, pointSet);
		pointSet.remove(uppestPoint);

		List<Point> leftUpperSet = getAllPointsOver(leftSidePoint, uppestPoint, pointSet);
		calculateBorder(leftSidePoint, uppestPoint, leftUpperSet, borderPoints);

		borderPoints.add(uppestPoint);

		List<Point> rightUpperSet = getAllPointsOver(uppestPoint, rightSidePoint, pointSet);
		calculateBorder(uppestPoint, rightSidePoint, rightUpperSet, borderPoints);
	}

	/**
	 * Bestimmt alle Punkte, welcher oberhalb einer Linie liegen,
	 * die durch zwei Punkte aufgespannt wird.
	 * 
	 * @param leftSidePoint linker Punkt der Linie
	 * @param rightSidePoint recher Punkt der Linie
	 * @param upperSet Lister der Punkte
	...
	/**
	 * Bestimmt den Punkt aus einer Punktliste, 
	 * welcher im 2-Dimensionalen Raum den größten 
	 * am weitesten von der Linie entfernt ist, welcher von
	 * zwei Punkten aufgespannt wird
	 * 
	 * @param leftSidePoint linker Punkt der Linie
	 * @param rightSidePoint rechter Punkt der Linie
	 * @param currentPointSet die Punktliste
