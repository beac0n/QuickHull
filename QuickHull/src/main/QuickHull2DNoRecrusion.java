...
public class QuickHull2DNoRecrusion extends QuickHull2D {
	...
	/**
	 * Teil des Quickhull-Algorithmuss, der nach und nach den Rand
	 ...
	 */
	@Override
	protected void calculateBorder(Point leftSidePoint, Point rightSidePoint, List<Point> pointSet, List<Point> borderPoints) {		
		...
		while(!leftSidePointStack.isEmpty()) {					
			...
			if (currentPointSet.size() > 0) {	
				...
				uppestPoint = getUppestPoint(currentLeftSidePoint, currentRightSidePoint, currentPointSet);
				currentPointSet.remove(uppestPoint);
					
				List<Point> leftUpperSet = getAllPointsOver(currentLeftSidePoint, uppestPoint, currentPointSet);
				
				borderPoints.add(uppestPoint);
				
				List<Point> rightUpperSet = getAllPointsOver(uppestPoint, currentRightSidePoint, currentPointSet);
